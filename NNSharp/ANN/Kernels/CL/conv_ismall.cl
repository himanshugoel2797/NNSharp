...
__kernel void conv_ismall(const int IN_OFF,
                          const int KERN_OFF,
                          const int OUT_OFF,
    ...
    __local float i_l[IN_D * IN_D];
    for(int idx = get_local_id(0); idx < IN_D * IN_D; idx += get_local_size(0))
        i_l[idx] = i[IN_OFF + idx];
...
#ifndef ROT_KERN
                acc += kern[KERN_OFF + (KERN_D - 1 - n0) * KERN_D + (KERN_D - 1 - n1)] * i_l[i_x * IN_D + i_y];
#else
                acc += kern[KERN_OFF + n0 * KERN_D + n1] * i_l[i_x * IN_D + i_y];
#endif
        }
