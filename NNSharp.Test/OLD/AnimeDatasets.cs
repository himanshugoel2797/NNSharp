...
namespace NNSharp.Test.OLD
{
    public class AnimeDatasets
    {
        ...
        const string TrainingDataPath = @"I:\Datasets\anime-faces\emilia_(re_zero)";
        const string TrainingDataPath_SMALL = @"I:\Datasets\anime-faces\emilia_small2";
        ...
        public void InitializeDataset()
        {
            ...
            if (TrainingFiles.Count == 0)
            {
                ...
                while (TrainingFiles.Count > 2000)
                {
            ...
            try
            {
                for (int h = 0; h < bmp.Height; h++)
                    for (int w = 0; w < bmp.Width; w++)
                    {
                        ...
                        bmp.SetPixel(w, h, Color.FromArgb((int)(img[offset + h * bmp.Width + w] * 255.0f), (int)(img[offset + h * bmp.Width + w] * 255.0f), (int)(img[offset + h * bmp.Width + w] * 255.0f)));
                    }
            }
            catch (Exception) { }
        ...
        /*
        public void SaveImageDiff(string file, float[] img, float[] img2)
        {
            var bmp = new Bitmap(Side, Side);
